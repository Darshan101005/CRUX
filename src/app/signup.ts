  ...
  template: `
    <div [class]="isDarkMode() ? 'dark' : ''">
      ...
      <div class="relative min-h-screen flex items-center justify-center px-4 py-20 overflow-x-hidden overflow-y-auto transition-colors duration-1000 bg-snow dark:bg-arctic-dark font-sans text-arctic-dark dark:text-snow">
        ...
        <!-- Glass Signup Panel -->
        <div class="glass-card bg-white/60 dark:bg-white/[0.03] border border-white/40 dark:border-white/5 w-full max-w-md p-10 rounded-3xl space-y-6 relative z-10 animate-in fade-in zoom-in duration-500 shadow-2xl mt-10 mb-10">
          <div class="text-center">
            <h2 class="text-4xl font-display font-bold text-arctic-dark dark:text-white mb-2">Create Account</h2>
            <p class="text-arctic-mid/70 dark:text-white/60 text-sm">Join the Arctic Intelligence Layer.</p>
          </div>
          ...
          <form [formGroup]="signupForm" (ngSubmit)="signUp()" class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <input formControlName="name" type="text" placeholder="Arctic Explorer" class="w-full bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-arctic-dark dark:text-white placeholder-arctic-mid/40 dark:placeholder-white/30 focus:outline-none focus:border-sky-500 dark:focus:border-[#3FD5FF] focus:shadow-[0_0_15px_rgba(14,165,233,0.3)] dark:focus:shadow-[0_0_15px_rgba(63,213,255,0.3)] transition-all">
              </div>
              @if (signupForm.get('name')?.errors?.['required'] && signupForm.get('name')?.touched) {
                <span class="text-xs text-red-500 ml-1">Name is required</span>
              }
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <input [type]="isPasswordVisible() ? 'text' : 'password'" (input)="onPasswordInput($event)" formControlName="password" placeholder="••••••••" class="w-full bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl pl-4 pr-12 py-3 text-arctic-dark dark:text-white placeholder-arctic-mid/40 dark:placeholder-white/30 focus:outline-none focus:border-sky-500 dark:focus:border-[#3FD5FF] focus:shadow-[0_0_15px_rgba(14,165,233,0.3)] dark:focus:shadow-[0_0_15px_rgba(63,213,255,0.3)] transition-all">
              ...
              <div class="bg-black/5 dark:bg-white/5 p-2 rounded-lg mt-2 text-[10px] space-y-1 transition-all">
                <div class="flex items-center gap-1.5" [ngClass]="reqs().length ? 'text-green-600 dark:text-green-400 font-bold' : 'text-arctic-mid/60 dark:text-white/40'">
                  <mat-icon class="text-[12px] w-3 h-3">{{reqs().length ? 'check_circle' : 'radio_button_unchecked'}}</mat-icon> 8+ characters
                </div>
                <div class="flex items-center gap-1.5" [ngClass]="reqs().upper ? 'text-green-600 dark:text-green-400 font-bold' : 'text-arctic-mid/60 dark:text-white/40'">
                  <mat-icon class="text-[12px] w-3 h-3">{{reqs().upper ? 'check_circle' : 'radio_button_unchecked'}}</mat-icon> Uppercase letter
                </div>
                <div class="flex items-center gap-1.5" [ngClass]="reqs().lower ? 'text-green-600 dark:text-green-400 font-bold' : 'text-arctic-mid/60 dark:text-white/40'">
                  <mat-icon class="text-[12px] w-3 h-3">{{reqs().lower ? 'check_circle' : 'radio_button_unchecked'}}</mat-icon> Lowercase letter
                </div>
                <div class="flex items-center gap-1.5" [ngClass]="reqs().number ? 'text-green-600 dark:text-green-400 font-bold' : 'text-arctic-mid/60 dark:text-white/40'">
                  <mat-icon class="text-[12px] w-3 h-3">{{reqs().number ? 'check_circle' : 'radio_button_unchecked'}}</mat-icon> Number
                </div>
                <div class="flex items-center gap-1.5" [ngClass]="reqs().special ? 'text-green-600 dark:text-green-400 font-bold' : 'text-arctic-mid/60 dark:text-white/40'">
                  <mat-icon class="text-[12px] w-3 h-3">{{reqs().special ? 'check_circle' : 'radio_button_unchecked'}}</mat-icon> Special character
                </div>
              </div>
            </div>

            <div class="space-y-2">
              <span class="block text-xs font-bold uppercase tracking-widest text-arctic-mid/80 dark:text-white/60 ml-1">Confirm Password</span>
              <div class="relative">
                <input [type]="isPasswordVisible() ? 'text' : 'password'" formControlName="confirmPassword" placeholder="••••••••" class="w-full bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-arctic-dark dark:text-white placeholder-arctic-mid/40 dark:placeholder-white/30 focus:outline-none focus:border-sky-500 dark:focus:border-[#3FD5FF] focus:shadow-[0_0_15px_rgba(14,165,233,0.3)] dark:focus:shadow-[0_0_15px_rgba(63,213,255,0.3)] transition-all">
              </div>
              @if (signupForm.hasError('passwordMismatch') && signupForm.get('confirmPassword')?.touched) {
                <span class="text-xs text-red-500 ml-1">Passwords do not match</span>
              }
            </div>
            
            <button type="submit" [disabled]="signupForm.invalid || isSuccess()" class="w-full py-3 mt-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg text-slate-900 disabled:opacity-50 disabled:cursor-not-allowed group" [ngClass]="isDarkMode() ? 'bg-gradient-to-r from-[#3FD5FF] via-white to-[#8e2de2] hover:shadow-[#3FD5FF]/50' : 'bg-gradient-to-r from-sky-400 via-sky-100 to-blue-600 hover:shadow-blue-500/50'">
              @if (!isSuccess()) {
          ...
          <div class="grid grid-cols-2 gap-4">
            <button class="bg-white/50 dark:bg-white/5 border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl p-3 flex justify-center items-center transition-colors gap-2 text-sm font-bold text-arctic-dark dark:text-white" title="Sign Up with Google">
            ...
            <button class="bg-white/50 dark:bg-white/5 border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl p-3 flex justify-center items-center transition-colors gap-2 text-sm font-bold text-arctic-dark dark:text-white" title="Sign Up with Telegram">
          ...
          <div class="text-center">
            <p class="text-sm text-arctic-mid/70 dark:text-white/60">
              ...
              <a routerLink="/login" class="text-sky-600 dark:text-[#3FD5FF] hover:underline font-bold">Login</a>
            </p>
          </div>
        </div>
        ...
        <!-- Success Overlay -->
        @if (isSuccess()) {
          <div class="fixed inset-0 z-[100] bg-white/80 dark:bg-arctic-dark/80 backdrop-blur-xl flex items-center justify-center animate-in fade-in duration-500">
            <div class="text-center space-y-6">
              <div class="w-24 h-24 rounded-full flex items-center justify-center mx-auto shadow-[0_0_50px_rgba(14,165,233,0.5)] dark:shadow-[0_0_50px_rgba(63,213,255,0.5)] animate-bounce font-bold text-white bg-gradient-to-r from-sky-400 to-blue-600 dark:from-[#3FD5FF] dark:to-[#8e2de2]">
                <mat-icon class="text-5xl">check</mat-icon>
              </div>
              <h2 class="text-4xl font-display font-bold text-arctic-dark dark:text-white">Welcome Aboard!</h2>
              <p class="text-arctic-mid/80 dark:text-white/60">Auto-logging you into your new command center...</p>
            </div>
          </div>
        }
            ...
            <div class="w-80 h-96 bg-white dark:bg-arctic-dark border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl mb-4 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <div class="flex-1 p-4 bg-snow dark:bg-arctic-dark/50 overflow-y-auto flex flex-col gap-3 text-left">
                <div class="bg-white dark:bg-white/10 p-3 rounded-xl rounded-tl-none border border-black/5 dark:border-white/5 text-sm text-arctic-dark dark:text-white w-[85%] shadow-sm">
                  Hi! Need help setting up your account?
                </div>
              </div>
...
export class Signup implements OnInit, OnDestroy {
  ...
  reqs = signal({ length: false, upper: false, lower: false, number: false, special: false });
